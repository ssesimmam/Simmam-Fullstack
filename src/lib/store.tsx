...
export const mapRemoteEventToAdminEvent = (
  ...
  mainCategory:
    remoteEvent.main_category || fallback?.mainCategory || initialEvents[0].mainCategory,
  venue: remoteEvent.venue || fallback?.venue || initialEvents[0].venue,
  time: remoteEvent.time_slot || fallback?.time || initialEvents[0].time,
  date: remoteEvent.date || fallback?.date || initialEvents[0].date,
  ...
  rules: (() => {
    const remoteRules = (remoteEvent as { rules?: unknown }).rules;
    return Array.isArray(remoteRules) && remoteRules.length > 0
      ? (remoteRules as string[])
...
export async function resolvePersistedEventId(event: AdminEvent): Promise<string> {
  ...
  const remoteMatch = remoteEvents.find(
    (candidate) => candidate.name.toLowerCase() === event.name.toLowerCase(),
  );

  return remoteMatch?.id || event.id;
}

const mapRemoteEventsToAdminEvents = (
  remoteEvents: Awaited<ReturnType<typeof fetchAdminEvents>>,
): AdminEvent[] => {
  const orderByName = new Map(
    initialEvents.map((event, index) => [event.name.toLowerCase(), index]),
  );
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const [houses, setHouses] = useState<House[]>(
    () =>
      readCachedValue<House[]>("simmam_houses") ||
      initialHouses.map((h) => ({
        ...h,
        points2026: h.points2026 ?? h.points2025 ?? 0,
      })),
  ...
  const [participants, setParticipants] = useState<Participant[]>(
    () => readCachedValue<Participant[]>("simmam_participants") || [],
  );
  ...
  useEffect(() => {
    ...
    if (storedEvents) {
      const parsedEvents = JSON.parse(storedEvents) as AdminEvent[];
      const reattachedEvents = parsedEvents.map((e) => {
    ...
    if (storedHouses) {
      const parsedHouses = JSON.parse(storedHouses) as House[];
      // Migration: If old data has points2025 but not points2026, migrate it
      const migratedHouses = parsedHouses.map((h) => ({
        ...h,
        points2026: h.points2026 ?? h.points2025 ?? 0,
      }));
    ...
    const eventsJob = (isAdminRoute ? fetchAdminEvents() : fetchEvents())
      .then((remoteEvents) => {
        if (remoteEvents.length > 0) {
          const mappedEvents = mapRemoteEventsToAdminEvents(
            remoteEvents as Awaited<ReturnType<typeof fetchAdminEvents>>,
          );
      ...
      .then(([remoteHouses, remoteLeaderboard]) => {
        if (remoteHouses.length > 0) {
          const leaderboardPointsByName = new Map(
            remoteLeaderboard.map((item) => [
              item.house_name.toLowerCase(),
              Number(item.total_points ?? item.points ?? 0),
            ]),
          );

          const mappedHouses = initialHouses.map((house) => {
            const remoteHouse = remoteHouses.find(
              (candidate) => candidate.name.toLowerCase() === house.name.toLowerCase(),
            );
            const points2026 =
              leaderboardPointsByName.get(house.name.toLowerCase()) ??
              Number(remoteHouse?.points ?? house.points2026 ?? 0);
      ...
      .then((remoteRegistrations) => {
        if (Array.isArray(remoteRegistrations)) {
          const mappedParticipants = remoteRegistrations.map((row) => ({
            ...
            status:
              (row.registration_status as "confirmed" | "pending" | "waitlisted") || "confirmed",
  ...
  const deleteEvent = (eventId: string) => {
    const updatedEvents = events.filter(
      (event) => event.id !== eventId && event.name.toLowerCase() !== eventId.toLowerCase(),
    );
    setEvents(updatedEvents);
    writeCachedValue("simmam_events", updatedEvents);
  };
